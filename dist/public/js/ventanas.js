...
editar1.forEach(btn => {
  btn.addEventListener("click", e => {
    ...
    fetch(`http://localhost:3000/usuario/${id}`).then(response => {
...
editar2.forEach(btn => {
  btn.addEventListener("click", e => {
    ...
    console.log("Fetching user data...");
    fetch(`http://localhost:3000/producto/producto/${id}`).then(response => {
      console.log("Response received:", response);
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
pagado.forEach(btn => {
  btn.addEventListener("click", e => {
    ...
    }).then(res => res.json()).then(esneider => console.log(esneider)).catch(error => console.log(error));
